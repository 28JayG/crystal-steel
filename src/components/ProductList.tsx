...
const ProductListContent: React.FC<ProductListProps> = ({ initialProducts, category = 'All' }) => {
  ...
  const categories = ['All', 'Steel', 'Pipes & Tubes', 'Structural', 'Machined Parts', 'Custom'];
  ...
  // Handle highlighted product from query parameters (e.g., from home page link)
  useEffect(() => {
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col p-6 flex-grow">
                  <Typography variant="h3" className="mb-2 text-base md:text-lg font-bold group-hover:text-amber-400 transition-colors">
                    {product.title}
                  </Typography>
                  <Typography variant="body" className="text-xs md:text-sm text-slate-400 leading-relaxed mb-6 flex-grow">
                    {product.description}
                  </Typography>

                  {/* Specifications Table */}
                  <div className="border-t border-slate-800/80 pt-4 mb-6">
                    <Typography variant="small" className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-2">
                      Technical Specifications
                    </Typography>
                    <div className="space-y-1.5">
                      {product.specs.map((spec, specIdx) => (
                        <div key={specIdx} className="flex justify-between text-xs border-b border-slate-800/40 pb-1 last:border-0">
...
export const ProductList: React.FC<ProductListProps> = (props) => (
  <Suspense fallback={<div className="text-center py-12 text-slate-400">Loading products list...</div>}>
