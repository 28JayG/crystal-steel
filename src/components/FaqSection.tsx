  ...
  return (
    <div className='w-full max-w-4xl mx-auto flex flex-col gap-8'>
      <div className='text-center mb-4'>
        ...
        <h2 className='text-3xl sm:text-4xl font-extrabold text-[#0B1628] tracking-tight relative inline-block mb-3'>
          Frequently Asked Questions
          <span className='absolute -bottom-2 left-1/2 -translate-x-1/2 w-14 h-1 bg-amber-500 rounded-full' />
        </h2>
      </div>
      ...
      <div className='flex flex-col gap-4'>
        {faqs.map((faq, idx) => {
          ...
          return (
            <div
              key={idx}
              className='rounded-2xl bg-[#F5F7FA] border border-[#DDE3EA] overflow-hidden transition-all'
            >
              <button
                onClick={() => toggleAccordion(idx)}
                type='button'
                className='w-full flex justify-between items-center p-6 text-left hover:bg-slate-100/70 transition-colors focus:outline-none cursor-pointer'
              >
                <span className='font-bold text-sm md:text-base text-[#0B1628] pr-4'>
                  {faq.question}
                </span>
                <ChevronDown
              ...
              <div
                className={`transition-all duration-300 ease-in-out ${
                  isOpen
                    ? 'max-h-60 border-t border-[#DDE3EA] opacity-100'
                    : 'max-h-0 opacity-0 pointer-events-none'
                }`}
              >
                <div className='p-6 text-sm text-[#526176] leading-relaxed bg-white'>
                  {faq.answer}
                </div>
              </div>
            </div>
